 ...
 * .. note::
 *
 *   This function, like its counterpart, naively unparses an IPv4 internet
 *   address into a string. This function needs to be rewritten such that
 *   endianness and IPv6 is handled.
 *
 * :param ip: The IPv4 internet address as an integer.
 * :returns: A dot-notation string of the internet address.
 */
const gallocy::string unparse_internet_address(uint64_t ip_address) {
  // Take half of the uint64_t representation after converting it to network
  // order (otherwise our IP address will be backwards!).
